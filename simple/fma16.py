...
QNAN16 = 0x7E00      # canonical quiet NaN
PINF16 = 0x7C00      # +infinity
NINF16 = 0xFC00      # -infinity
...
def pack_inf(sign):
    """Return +inf (sign=0) or -inf (sign=1) in binary16."""
    return PINF16 if sign == 0 else NINF16        
...
def pack_from_sum(Sm, Se, Ss, verbose=True):
    ...
    print_stage("clug", Sm_shift=Sm_shift, frac11=frac11, mant10=mant10)
...
def fma_binary16(x, y, z, FOpCtrl=0, verbose=True):
    ...
    # -----------------------
    # NaN + Inf/Zero handling
    # -----------------------

    # Any NaN operand -> qNaN
    if unpack_out['XNaN'] or unpack_out['YNaN'] or unpack_out['ZNaN']:
        if verbose: print_stage("SpecialCase", reason="NaN")
        return {'result': QNAN16}

    # Inf * 0 or 0 * Inf -> invalid -> qNaN
    if (unpack_out['XInf'] and unpack_out['YZero']) or \
       (unpack_out['YInf'] and unpack_out['XZero']):
        if verbose: print_stage("SpecialCase", reason="Inf*Zero")
        return {'result': QNAN16}

    # X or Y is Inf (non-NaN, non-Inf*0 cases)
    if unpack_out['XInf'] or unpack_out['YInf']:
        Ps = unpack_out['Xs'] ^ unpack_out['Ys']  # product sign
        ...
        # Otherwise result is +/-Inf with sign Ps
        result_inf = pack_inf(Ps)  # or: PINF16 if Ps==0 else NINF16
